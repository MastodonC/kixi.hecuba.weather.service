...
: ${SERVICE_JAVA_OPTS:='-XX:+UseG1GC -server'}

echo "Starting kixi.hecuba.weather.service"
echo "Using user: ${HECUBA_USER}"
echo "Searching on Hecuba programme: ${PROGRAMME_ID}"

/usr/bin/java $SERVICE_JAVA_OPTS \
              "-Xmx${XMX}m" \
              -jar /opt/weather-service.jar \
              -c/opt/config.edn -u${HECUBA_USER} -p${HECUBA_PASS} -a${PROGRAMME_ID}
